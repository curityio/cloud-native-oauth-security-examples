...
export class LoginRequestResponse {
    ...
    public async initialize(): Promise<string> {
        ...
        const options = {
            port: 3000,
            host: Configuration.loopbackHostname,
        };
        this.port = await getPort(options);

        return `http://${Configuration.loopbackHostname}:${this.port}/callback`;
    }
    
    /*
     * Do the plumbing work to send the request URL and receive the response URL
     */
    public async execute(authorizationRequestUrl: string): Promise<URL> {

        // Start an HTTP server and listen for the authorization response on a loopback address
        this.httpServer = http.createServer(this.handleLoopbackHttpRequest);
        this.httpServer.listen(this.port, Configuration.loopbackHostname);
    ...
    private async handleLoopbackHttpRequest(
        ...
        response.write('Login completed successfully');
        response.end();
