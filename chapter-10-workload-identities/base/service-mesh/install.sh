...
#
# Create an OpenSSL wildcard certificate for the API gateway's external URLs
#
...
if [ $? -ne 0 ]; then
  echo '*** Problem encountered creating the Kubernetes TLS secret for the ingress controller'
  exit 1
fi
