...
if [ ! -f '../../license.json' ]; then
  echo '*** Please provide a license file for the Curity Identity Server in the chapter-10-workload-identities folder'
  exit 1
fi
...
#
# Fix up service accounts where not currently supported by the Helm chart
#
kubectl -n authorizationserver patch deployment curity-idsvr-admin --patch-file ./resources/admin-service-account-patch.json
if [ $? -ne 0 ]; then
